...
def main():
    parser = argparse.ArgumentParser(description="Naive cDBG builder & querier (V2-compliant)")
    sub = parser.add_subparsers(dest="cmd")

    # BUILD
    b = sub.add_parser("build", help="Construire un index naïf")
    ...
    b.add_argument("-o", dest="output_index", required=True, help="Fichier de sortie (index pickle)")

    # QUERY
    q = sub.add_parser("query", help="Interroger un index")
    q.add_argument("-q", dest="query_fasta", required=True, help="Fichier FASTA des requêtes")
    q.add_argument("-i", dest="input_index", required=True, help="Index sérialisé (pickle)")
